...
Everything here is applied at container start by `scripts/tp3-prelude.sh`, which runs the patch
scripts and then execs the server. **A patch whose anchor no longer matches stops the rank.** That is
deliberate: a half-patched stack is exactly the failure mode that serves confident, wrong answers.
...
| `vocab_size` | 154,880 | pads to 154,880, /2 = 77,440 | `padding_size` 64 → **192**, giving 154,944, /3 = 51,648 | `patches/tp3/patch-vllm-tp3.py` |
| shared expert (`moe_intermediate_size × n_shared_experts`) | 2,048 | 1,024 per rank | **padded to 2,112** → 704 per rank | `patches/tp3/patch-vllm-tp3.py` |
...
The shared expert is native BF16, so zero-extending it is a real no-op: 2,048 → 2,112 (a multiple of
`lcm(64, 3) = 192`) adds 64 zero columns and the maths is unchanged.

The routed experts are EXL3 trellis. A trellis column is meaningful only on a 128-element Hadamard
boundary, and **a zero-extended trellis is not a zero-extended weight** — it decodes to noise. Nor is
2,048/3 an integer. So the routed experts are distributed whole under expert parallelism and never
padded and never sliced. This distinction is the single most important idea on this page: padding is
the right tool for the unquantized parts of this model and the wrong tool for the quantized parts.
...
So `patches/tp3/pad-tp3.py` writes a **sidecar directory**: relative symlinks to every file of the
downloaded checkpoint, plus one rewritten `config.json`.
...
```
patches/tp3/pad-tp3.py /var/tmp/glm-5.3-flash-tr3-4bpw /var/tmp/glm-5.3-flash-tr3-4bpw-tp3 --tp 3
```
...
| `VocabParallelEmbedding` | `divide(num_embeddings_padded, tp_size)` | 154,880 % 3 = 2 | `patch-vllm-tp3.py`: `padding_size` 64 → 192 |
...
`patches/tp3/preflight-tp3.py` runs inside the container, against **whatever the launcher actually
mounted** rather than against what the environment file says it mounted, and it owns the
...
- It checks every shape in §1 individually.
...
```
patches/tp3/preflight-tp3.py --model /var/tmp/glm-5.3-flash-tr3-4bpw-tp3 --tp 3 --ep 1
```

### Prove the pad is zero, not just that it exists

For the drafter, the stronger check runs after load: read the fabricated rows of every layer's
