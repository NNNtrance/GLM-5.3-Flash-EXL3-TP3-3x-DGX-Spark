...
image `exl3-zeus:f4987cf`, KV `fp8`, DFlash2 draft k=7, `--block-size 256`,
...
temperature 0, **reasoning effort `low`**, 5 September 2026. Speed is the median of sweep rounds 3–5
with rounds 1–2 discarded ([09](09-measurement-protocol.md)). Raw tables in
...
| | value |
|---|---|
| C1 aggregate / per stream | **54.4** / 61.8 tok/s `[measured-here]` |
| C2 / C4 / C6 / **C8** aggregate | 80.1 / 114.6 / 136.8 / **161.8** tok/s `[measured-here]` |
| C8 per stream | 25.2 tok/s `[measured-here]` |
| TTFT, C1 / C8 | 0.47 / 1.14 s `[measured-here]` |
| Draft acceptance / accepted tokens per step | 61–64 % / 5.3 `[measured-here]` |
| Prefill, fresh unseen ~8.2K prompts (median of 3) | **1,704** tok/s `[measured-here]` |
| Prefill, warm repeated 7K prompt | 1,475 tok/s `[measured-here]` |
| Mixed load: decode while a 7K prompt lands / its TTFT | 7.0 tok/s / 4.9 s `[measured-here]` |
| KV pool | **4,484,848** tokens (4.48 concurrent 1M-token requests) `[measured-here]` |
| Weights per node | 54.86 GiB `[measured-here]` |
| Boot, container start → API ready | **274 s** `[measured-here]` |
| Free host RAM / swap at rest | 10.9 / 12.2 / 12.1 GiB · ~0.1 GiB `[measured-here]` |
| Quality gates, cold and warm | 10/10 · 12/12 `[measured-here]` |

By content type at a single stream: code **47.9**, math **59.0**, JSON **57.7**, prose **22.4** tok/s,
with draft acceptance 46 / 56 / 55 / **13 %** `[measured-here]`. Prose is where a k=7 draft is
wasted, and it is the one category where a shallower draft wins
([04](04-dflash2-port.md) §6).
...
| **+ fast-boot sidecar (production)** | **54.4** | **114.6** | **161.8** | **1,704** | **4,484,848** | [08](08-fast-boot.md) |

Two of those rows are the interesting ones. **The third node initially made the machine slower**, by
8–29 %, and that was a one-line kernel bug rather than a cost of the arrangement. And **the largest
single jump in the KV pool cost no memory at all** — it was a per-request block counter, not bytes.
...
|---|---|---|
| C1 aggregate | 54.4 | **57–60** |
| C8 aggregate | **161.8** | 150 |
| prefill, 7K | 1,475 | **1,585** |
| TTFT C1 | 0.47 s | **0.38 s** |
| KV pool | **4,484,848 @ 0.80** | 4,321,739 @ 0.88 |
...
**EXL3 is ahead on aggregate throughput, on memory, and on boot; behind on single-stream decode and
on prefill.** The KV comparison is the sharper one: EXL3 reaches a slightly larger pool at
`gpu-memory-utilization 0.80` than NVFP4 reaches at 0.88, which means it has headroom NVFP4 does not.

Two caveats that matter. The MMLU numbers are not comparable — one is a full run and one is a
1,995-question sample measured on a two-node arrangement. And `NCCL_MAX_NCHANNELS=8`, which is worth
+13 % at C8 here, uses the same plugin over the same fabric and has **not** been applied to the NVFP4
stack `[not tested]`; if it transfers, that comparison moves.
...
|---|---|---|---|---|---|
| Decode C1, DFlash2 k=7 | 61.8 tok/s per stream, 5.47 tok/step → 88.5 ms/step | 8 verify tokens → ~19 of 96 experts per layer: 10.6 GB expert + 5.3 GB dense ≈ **15.9 GB** | ≈ **180 GB/s** | ≈ **74 %** | ≈ 2 % |
| Decode C8, DFlash2 k=7 | 161.8 tok/s total, 42.5 accepted tok/step → 263 ms/step | 64 verify tokens → ~80 of 96 experts per layer: 44.3 GB + 5.3 GB ≈ **49.6 GB** | ≈ **189 GB/s** | ≈ **78 %** | ≈ 5 % |
| Prefill, 2048-token chunk | 1,704 tok/s → 1.20 s per chunk | every expert touched: 53.3 GB + 5.3 GB ≈ **58.6 GB** | ≈ **49 GB/s** | ≈ **20 %** | ≈ **23 %** (≈ 23 TFLOPS/node) |

**Reading.** Decode with the draft already sits at roughly three quarters of the memory roof, so
...
all-reduce at 21.9 % `[measured-here]`. At decode the BF16 dense GEMMs are the **largest single
...
- **Prefix caching.** With a 3,328-token attention block, our benchmark prompts never fill one, so
  ...
  `[measured-here]`.
- **Long-context behaviour at scale.** The KV pool supports 4.48 concurrent million-token requests
  and we never drove it past 13 % usage `[measured-here]`.
