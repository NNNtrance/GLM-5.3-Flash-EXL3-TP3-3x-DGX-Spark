...
def _flashinfer_topk() -> Callable[..., tuple[torch.Tensor, torch.Tensor]] | None:
    ...
    torch.topk.
    """
    # HAREM escape hatch. The DFlash2 candidate selector is the only caller
    # of this top-k, and it runs over a 154880-entry vocabulary. If the
    # FlashInfer radix kernel ever misbehaves on GB10/sm_121 (this platform
    # has needed shared-memory workarounds elsewhere, cf. persistent_topk),
    # set HAREM_DFLASH2_FORCE_TORCH_TOPK=1 to fall back to torch.topk and
    # re-measure acceptance. Default is unchanged upstream behaviour.
    if os.environ.get("HAREM_DFLASH2_FORCE_TORCH_TOPK", "0") == "1":
        logger.info_once(
            "HAREM_DFLASH2_FORCE_TORCH_TOPK=1: vocab-parallel top-k uses "
            "torch.topk instead of the FlashInfer radix kernel."
        )
        return None
