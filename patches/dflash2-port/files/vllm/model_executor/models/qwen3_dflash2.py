...
class DFlash2Qwen3Model(DFlashQwen3Model):
    ...
    def __init__(
        ...
        *,
        ...
        self._harem_check_port_assumptions(vllm_config)
    ...
    def _harem_check_port_assumptions(self, vllm_config: VllmConfig) -> None:
        """Fail closed on the two assumptions this port deliberately does not carry.

        See PATCHES.md. Both failures are otherwise SILENT: the engine boots,
        answers correctly-looking text, and only acceptance collapses -- which is
        exactly how the NVFP4 stack lost a week to a 22-head decode kernel.

        (1) Quantized drafter. Upstream b389ac294 builds the DFlash2 convolution
            and candidate-selector projections with ``quant_config=None``, i.e. it
            assumes an unquantized drafter. The LIL fork later threaded the draft
            quant config into them (da4d7be6c) so a serialized MXFP8 (B12X/NVFP4)
            drafter runs those projections through the ModelOpt kernel. That path
            is NOT ported: this stack serves the BF16
            incoai/GLM-5.3-Flash-DFlash2 checkpoint, where quant_config is None
            and the two forms are identical. A quantized drafter would silently
            get BF16 projections over quantized weights.
            TO PORT IT: take the qwen3_dflash2.py hunks of da4d7be6c (thread
            ``quant_config`` into DFlashGroupedConv and CandidateSelector) plus
            its qwen3_dflash.py hunks (_build_fused_kv_buffers /
            process_weights_after_loading), then relax this check.

        (2) Head counts not divisible by the TP size. This checkpoint copy is the
            TP=2 one (32 heads / 8 KV heads). The stock incoai drafter is 36/9,
            which divides by 3 but not by 2. A TP=3 run therefore needs the 36/9
            drafter, and the LIL fork pads the loader to 36/9 for that case.
            HOOK FOR THE TP=3 AGENT: that padding is a separate task; this check
            is where a wrong-shaped drafter must stop, rather than silently
            producing a degraded draft.
        """
        if self.quant_config is not None:
            raise NotImplementedError(
                "DFlash2 port: quantized drafter is not supported. The draft "
                f"quant config is {type(self.quant_config).__name__}, but the "
                "convolution and candidate-selector projections are built "
                "unquantized (upstream b389ac294 form). Serve the BF16 "
                "incoai/GLM-5.3-Flash-DFlash2 checkpoint, or port the "
                "qwen3_dflash2.py hunks of fork commit da4d7be6c first. "
                "See PATCHES.md."
            )
