...
def _pool_bytes_per_block(
    ...
    if (glm5 := _glm5_next_tensor_layout(kv_cache_groups)) is not None:
        ...
        return (
            ...
            + _harem_draft_bytes_per_block(draft_groups)
        )
...
def _glm5_next_tensor_layout(
    ...
    # A DFlash draft contributes its own independent group(s) (plain attention
    # specs, neither UniformType nor Mamba). They take no part in GLM-5.3's slot
    # sharing but do consume block ids from the shared pool, so they are
    # reported back to the callers rather than ignored.
    draft_groups = [
        g
        for g in kv_cache_groups
        if not isinstance(g.kv_cache_spec, (UniformTypeKVCacheSpecs, MambaSpec))
    ]
    ...
    if len(uniform_groups) + len(mamba_groups) + len(draft_groups) != len(
        ...
        return None
    # Fail closed: only plain attention groups may ride alongside; anything else
    # is an unmodelled layout and must fall back rather than be mis-accounted.
...
def get_kv_cache_config_from_groups(
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        kv_cache_tensors = [
            ...
            # DFlash draft layers: one private tensor each, sized to the same
            # shared block-id space.
            KVCacheTensor(
...
def _harem_partition_dflash_draft_specs(
    vllm_config: VllmConfig,
    kv_cache_spec: dict[str, KVCacheSpec],
) -> tuple[dict[str, KVCacheSpec], dict[str, KVCacheSpec]]:
    """Split the DFlash draft's appended KV layers from the target's.

    Adapted from the LIL fork's ``_partition_dflash_draft_specs``. The DFlash
    head is built with ``start_layer_id = <target layer count>``, so its layers
    carry indices at or above that count.

    Why this is needed: GLM-5.3 has its own grouping path
    (``_get_kv_cache_groups_glm5_next``) in which mamba layers co-own the MLA
    slot tensors and the kpool tail co-owns the indexer tensors. That path
    requires every attention spec to be an MLAAttentionSpec. A DFlash draft adds
    sliding-window attention layers, which disqualifies it, and the model then
    falls through to the generic multi-group path -- which calls
    ``unify_kv_cache_spec_page_size`` and dies, because the kpool indexer page
    (33 B/token) can never be scaled to the MLA/mamba page (512 B/token) by
    integer block scaling, and its backend does not index by block stride.

    Grouping the target and the draft separately keeps the target's grouping
    bit-for-bit what it is without a drafter, and gives the draft its own
    independent group.
    """
    from vllm.model_executor.models.utils import extract_layer_index
...
def _harem_draft_bytes_per_block(
    ...
    """Bytes one shared block id costs for the DFlash draft's own group(s).

    Every group draws block ids from the same pool, so the draft's layers add
    their page to the per-block cost exactly like the MLA and indexer layers do.
    Leaving this out would over-report how many blocks fit and leave the draft's
    tensors unallocated.
    """
    return sum(
...
def get_kv_cache_groups(
    ...
    # Group a DFlash draft's KV layers independently of the target's, so the
    # target keeps whichever specialised path it would take on its own (for
    # GLM-5.3 that is _get_kv_cache_groups_glm5_next). See
    # _harem_partition_dflash_draft_specs.
