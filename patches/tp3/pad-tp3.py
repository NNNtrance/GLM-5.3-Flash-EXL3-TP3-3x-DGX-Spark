#!/usr/bin/env python3
"""Build a TP=3 sidecar view of a GLM-5.3-Flash checkpoint.
...
NOT padded  vocab_size            154880
    ...
    module. The fix there is ``padding_size = lcm(64, tp) = 192`` (154880 ->
    154944), applied by ``patch-vllm-tp3.py``, not a config edit.
...
-----
  # target
  ./pad-tp3.py /var/tmp/glm-5.3-flash-tr3-4bpw \
               /var/tmp/glm-5.3-flash-tr3-4bpw-tp3 --tp 3

  # DFlash2 drafter (flat Qwen3-style config, GQA)
  ./pad-tp3.py /var/tmp/dflash2-draft /var/tmp/dflash2-draft-tp3 --tp 3 --draft
...
def main() -> None:
    ...
    if args.draft:
        ...
        print(f"  wrote {dst / CONFIG}: heads {t['num_attention_heads']}, "
              ...
              f"{ceil_to(int(t['vocab_size']), 64 * args.tp // gcd(64, args.tp))})")
    print("OK")
