# patches/tp3full — the full-scope patch tree (production configuration 9)

This is the second patch tree. [`patches/tp3/`](../tp3/) serves the
...
```
for f in patches/tp3full/*.py patches/tp3full/overlay/cuda_exl3/*.py; do b=${f#patches/tp3full/}; [ -e "patches/tp3/$b" ] && { cmp -s "$f" "patches/tp3/$b" && echo "same $b" || echo "DIFFERS $b"; }; done
```
...
`ln patches/tp3full/tp3full-prelude.sh ~/exl3-zeus/tp3full/tp3-prelude.sh`, not `cp`.
...
A1–A8 are the TP=2 patch, [`patches/tp2/patch-fullscope-tp2.py`](../tp2/patch-fullscope-tp2.py),
