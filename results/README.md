...
| `memory/indexer-workspace-ab.md` | **The sparse-indexer K-gather workspace, and what bounding it is worth.** The 4.92 GiB buffer inside the "non-torch" residual: where the size comes from, why the profiler charges it to the KV pool, the bound derived from the consumers, the four layers that make an under-size a loud failure, and a same-session control-against-patched A/B — workspace 5,036.40 → 513.00 MB, **pool +10.25 %** at the same memory fraction, gates cold and warm, a 969,468-token single request, eight concurrent long-context lanes, five concurrency levels and the swap picture. A **candidate**, not production, and §7 says what promotion costs. |
